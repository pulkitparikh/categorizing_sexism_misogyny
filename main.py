                                                        ...
                                                        for run_ind in range(conf_dict_com["num_runs"]):
                                                            ...
                                                            if run_ind < len(mod_op_list_save_list):
                                                                ...
                                                                for m_ind, (loss_func, cw, out_vec_size, trainY) in enumerate(zip(loss_func_list, cw_list, out_vec_size_list, trainY_list)):
                                                                    mod_op, att_op = train_predict(word_feats, sent_enc_feats, trainY, data_dict, model_type, num_cnn_filters, rnn_type, fname_part, loss_func, cw, nonlin, out_vec_size, rnn_dim, att_dim, max_pool_k_val,stack_rnn_flag,cnn_kernel_set, m_ind, run_ind, conf_dict_com["save_folder_name"], conf_dict_com["use_saved_model"], conf_dict_com["gen_att"], conf_dict_com["LEARN_RATE"], conf_dict_com["dropO1"], conf_dict_com["dropO2"], conf_dict_com["BATCH_SIZE"], conf_dict_com["EPOCHS"], conf_dict_com["save_model"], conf_dict_com["save_trained_mod"])
